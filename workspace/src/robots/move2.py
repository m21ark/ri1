...
class Move2(Node):
    ...
    def __init__(self):
        ...
        self.lidar_sub1 = self.create_subscription(LaserScan, '/robot2/static_laser1', self.reactive1, 10)
        self.lidar_sub2 = self.create_subscription(LaserScan, '/robot2/static_laser2', self.reactive2, 10)
        self.odom_sub = self.create_subscription(Odometry, '/odom2', self.odom_logger, 10)
        self.col_sub = self.create_subscription(Collisions, '/collisions2', self.collision_logger, 10)
    ...
    def odom_logger(self, data: Odometry):
        ...
        if self.startPos == None:
            ...
            print(f'[Start Position R2]: {self.startPos}')
        else:
            ...
            if ended:
                ...
                print(f'[End Position R2]: {endPos}')
                print(f'[Number of Collisions R2]: {self.num_collisions}')
                print(f'[Average Velocity R2]: {sum(self.velocities) / len(self.velocities):.2f}')
                print(f'[Average Distance to Wall R2]: {sum(self.distances_to_wall) / len(self.distances_to_wall):.2f}')
                print(f'[Time Taken R2]: {time.time() - self.startTime:.2f} seconds')
                self.stop()
    ...
    def reactive1(self, msg: LaserScan):
        ...
        self.wall_ranges = msg.ranges # Store the wall ranges to occlude the other robot
        
    def reactive2(self, msg: LaserScan):
        ranges = [x if not np.isnan(x) else float('inf') for x in msg.ranges]
        
        # occlude the current ranges with the wall ranges if wall range is less than the current range
        # for i in range(len(ranges)):
        #     if self.wall_ranges[i] < ranges[i]:
        #         ranges[i] = 69
