...
def main():
    ...
    if closest_following_index is not None:
        plt.plot(robot2_x_vals[:closest_following_index], robot2_y_vals[:closest_following_index], marker='o', linestyle='-', color='orange', markersize=2, label='robot 2 Wall Path')
        # Plot the path after it started following with purple line
        plt.plot(robot2_x_vals[closest_following_index:], robot2_y_vals[closest_following_index:], marker='o', linestyle='-', color='red', markersize=2, label='robot 2 Following Path')
    else:
    ...
    img1.save('logs/robot_path_map.png')

if __name__ == '__main__':
    try:
        main()
    except:
        print('An error occurred while generating the statistics.')
